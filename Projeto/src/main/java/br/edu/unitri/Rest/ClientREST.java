...
public abstract class ClientREST<T> {
	...
	private String getBaseURI() throws Exception{
		return UtilBean.getAbsolutePathApp() + "/rest/";
	}
	...
	protected String editar(T entity) throws Exception{		
		Response response = targetWeb("editar").request().put(Entity.entity(entity, MEDIA_TYPE));		
		handlerResponseStatus(response);
		return response.readEntity(String.class);	
	}
